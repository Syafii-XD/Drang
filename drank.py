...
def menu():
    ...
    if asw == "":
      ...
      jalan('%s╚══[%s!%s] %sSampai Jumpa'%(M,P,M,P))
      os.system('rm -rf token.txt')
      os.system('rm -rf cookie.txt')
      exit()
    else:
...
def _main_random_():
    files = 'dump_random.json'
    try:os.remove(files)
    except:pass
    print('%s╠══[%s01%s] %sDump ID Old'%(O,P,O,P))
    print('%s╠══[%s02%s] %sDump ID New'%(O,P,O,P))
    _cok_ = input('%s╠══[%s••%s] %sPilih : '%(O,P,O,P))
    print('%s║'%(O))
    if _cok_ in ['1','01']:_random_old_(files);print('')
    elif _cok_ in ['2','02']:_random_new_(files);print('')
    else:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu()
def _random_old_(files):
    try:
        open(files,'a')
        _jum_ = int(input("%s╠══[%s•%s] %sMasukkan Jumlah : "%(O,P,O,P)))
        _old_ = input("%s╠══[%s•%s] %sID Old Berapa Digit : "%(O,P,O,P))
        print('%s╠══[%s•%s] %sContoh : 1 / 12 / 15 / 18 / 19'%(O,P,O,P))
        _dig_ = input('%s╠══[%s•%s] %sMasukkan ID Depan : '%(O,P,O,P))
        _idg_ = int(_old_) - int(len(_dig_))
        _awa_ = int(str('1'*_idg_))
        _akh_ = int(str('9'*_idg_))
        print('%s║'%(O))
        print("%s╠══[%s•%s] %sTekan ctrl+c Untuk Berhenti Dump"%(O,P,O,P))
        for i in range(_jum_):
            x = random.randint(_awa_,_akh_)
            id = (str(_dig_)+str(x))
            try:open(files,'a+').write('%s\n'%(id))
            except :pass
            print("\r%s╠══[%s•%s] %sSedang Generate %s ID"%(O,P,O,P,len(open(files).read().splitlines())),end='');sys.stdout.flush()
    except:print('')
def _random_new_(files):
    try:
        open(files,'a')
        _jum_ = int(input("%s╠══[%s•%s] %sMasukkan Jumlah : "%(O,P,O,P)))
        print('%s╠══[%s•%s] %sContoh : 10005 / 100005 / 1000005'%(O,P,O,P))
        _dig_ = input('%s╠══[%s•%s] %sMasukkan ID Depan : '%(O,P,O,P))
				...
				if _key in key:
					...
					if ter in ["m","M"]:
						with ThreadPoolExecutor(max_workers=30) as coeg:
							...
							id = open(files,'r').read().splitlines()
							started()
							for user in id:
								uid, name = user.split("<=>")
